...
export async function POST(request: NextRequest) {
  try {
    ...
    ${dados.dividas.map((divida, index) => `${index + 1}. Dívida de R$ ${divida.valor} com taxa de juros de ${divida.taxaJuros}% ao mês e prazo de ${divida.prazo}`).join('\n')}
    ...
    const completion = await groq.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'Você é um consultor financeiro especializado em ajudar pessoas a saírem das dívidas e melhorarem sua situação financeira.'
        },
        {
          role: 'user',
          content: prompt
    ...
    try {
      ...
      planoFinanceiro = { erro: 'Falha ao analisar resposta da IA', conteudoBruto: completion.choices[0]?.message?.content };
    }
    ...
    const caminhoArquivo = path.join(process.cwd(), 'public', 'planos_financeiros', nomeArquivo)
